  ...
  const baias = CONFIG.baias.map(baiaConfig => {
    ...
    const origem = origens.reduce((acc, item) => {
      ...
      acc.boletos += pegarInteiro(
        ...
        item.boleto ??
        item.totalBoletos ??
        item.matriculasBoleto ??
        item.matrículasBoleto ??
        0
      );

      acc.vendasConfirmadas += pegarInteiro(
        item.vendasConfirmadas ??
        item.quantidade ??
        item.qtd ??
        item.vendas ??
        item.totalVendas ??
      ...
      if (Array.isArray(item.membros)) {
        acc.membros.push(...item.membros);
      } else if (Array.isArray(item.vendedores)) {
        acc.membros.push(...item.vendedores);
      } else if (Array.isArray(item.consultores)) {
        acc.membros.push(...item.consultores);
      } else {
        const nomeMembro = item.membro ?? item.vendedor ?? item.consultor ?? item.nome_membro ?? item.nomeMembro;
        if (nomeMembro) {
          acc.membros.push({
            nome: nomeMembro,
    ...
    return montarBaiaVisual(baiaConfig, origem);
  });
